...
span = 10; n_point_per_side = span*10
...
MN_test=1.2*get_M_N_waves(r,theta,phi, -1,12, Omega, Eps, Mu, kind=1, return_Cartesian=True)[0]+7.7*get_M_N_waves(r,theta,phi, -2,3, Omega, Eps, Mu, kind=1, return_Cartesian=True)[0]
Col_1=MN_test[:,:,:,0]
Col_2=MN_test[:,:,:,1]
Col_3=MN_test[:,:,:,2]
...
# ERROR Check
#np.sum(np.abs(E_calculate-E_test)/E_calculate+E_test)
...
ax_2.plot(place_hod,E_test.imag)
ax_5.plot(place_hod,E_calculate.imag-E_test[:,0].imag)

ax_3.plot(place_hod,E_calculate.real)
ax_4.plot(place_hod,E_test.real)
ax_6.plot(place_hod,E_calculate.real-E_test[:,0].real)
...
initial_guess=np.ones((Row_vec.shape[1],1))
