...
alpha_max=get_N_SphericalWaves_given_L_max(N_max)
 ...
 # I have just made 'alpha_max' to be at the column

MN_Omega_start=1e14
MN_Omega_end=10e14
MN_Omega_len=3

MN_Omega_array=np.linspace(MN_Omega_start,MN_Omega_end,MN_Omega_len)


from progressbar import ProgressBar
pbar = ProgressBar()
for o in range(0,MN_Omega_len):
    Current_Omega=MN_Omega_array[o]
    Row_vec=np.zeros(  (3*totaL_len, alpha_max+1)  )*1j
    ...
    for n in (range(1,N_max+1)):
        ...
        for m in (range(-n,n+1)):
            #print(m)
            #print(m)
            MN=get_M_N_waves(r,theta,phi, m,n, Current_Omega, Eps, Mu, kind=1, return_Cartesian=True)
    ...
    if o==0:
        ...
        Coff_matrix=np.hstack([Coff_matrix,Row_vec])
